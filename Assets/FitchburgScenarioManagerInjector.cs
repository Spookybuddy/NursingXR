...
public class FitchburgScenarioManagerInjector : CoreInjectorComponent<IScenarioManager>
{
    public override IScenarioManager GetSingleton()
    {
        var gigxrCore = FindObjectOfType<GIGXRCore>();
        var provider = gigxrCore.DependencyProvider;

        var stageManager = new StageManager();
        var assetManager = new FitchburgAssetManager
        (
            calibrationRoot: provider.GetDependency<ICalibrationRootProvider>(),
            gigxrCore.AssetTypeLoader,
            gigxrCore.AddressablesGameObjectLoader,
            stageManager,
            UnityScheduler.Instance,
            provider.GetDependency<ProfileManager>(),
            runtimeExpeditedFunction: async monoBehaviours =>
            {
                await new DependencyInjector(provider).InjectIntoMonoBehavioursMethods(monoBehaviours);
            },
            featureManager: provider.GetDependency<IFeatureManager>()
        );

        gigxrCore.AddressablesGameObjectLoader.SetManager(assetManager);
...
public class FitchburgAssetManager : IGigAssetManager, IDisposable
{
    ...
    public FitchburgAssetManager(ICalibrationRootProvider calibrationRoot, IAssetTypeLoader assetTypeLoader,
