...
namespace GIGXR.Platform.Utilities.SerializableDictionary
{
    public class SerializableDictionaryPropertyDrawer : PropertyDrawer
    {
        ...
        )
        {
            ...
            if (property.isExpanded)
            {
                ...
                foreach (var entry in EnumerateEntries(keyArrayProperty, valueArrayProperty))
                {
                    ...
                    float lineHeight = DrawKeyValueLine
                        (
                            keyProperty,
                            valueProperty,
                            linePosition,
                    ...
                    if (i == conflictState.conflictIndex &&
            ...
            foreach (var entry1 in EnumerateEntries(keyArrayProperty, valueArrayProperty))
            {
                ...
                foreach (var entry2 in EnumerateEntries
                    (
                        keyArrayProperty,
                        valueArrayProperty,
                        i + 1
        ...
        static float DrawKeyValueLineSimple
        (
            SerializedProperty keyProperty,
            SerializedProperty valueProperty,
            string keyLabel,
        ...
        static float DrawKeyValueLineExpand
        (
            SerializedProperty keyProperty,
            SerializedProperty valueProperty,
            Rect linePosition
        ...
        {
            if (value1 is Dictionary<string, object> &&
                value2 is Dictionary<string, object>)
            {
        ...
        static IEnumerable<EnumerationEntry> EnumerateEntries
        (
            SerializedProperty keyArrayProperty,
            SerializedProperty valueArrayProperty,
            int startIndex = 0
        )
        {
            if (keyArrayProperty.arraySize > startIndex)
            {
                ...
                do
                {
                    yield return new EnumerationEntry
                        (
                            keyProperty,
                            valueProperty,
                            index
